(ns user
  ...
  (:require
    ...
    [mainside.finders-keepers.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (mainside.finders-keepers.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (mainside.finders-keepers.config/system-config {:profile :test})
                                  (ig/prep)))))
